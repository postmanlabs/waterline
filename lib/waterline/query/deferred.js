...
var Deferred = module.exports = function(context, method, criteria, values, _transactionId) {
  ...
  try {
    ...
    if(this._context.waterline.schema[attr.references].junctionTable && hasOwnProperty(attr, 'on')) {
      ...
      if(ref) {
        ...
        // @todo: fetch extended columnName for `userSelect`
    ...
    // ensure that all join foreign keys are present in criteria that came from .populates
    _.isArray(this._criteria.select) && (this._criteria.select.indexOf(keyName) === -1) &&
      this._criteria.select.push(keyName);
...
Deferred.prototype.populateCount = function (association, keyName, criteria) {
  var self = this,
    count,
    parentKey,
    pk,
  ...
  if (!_.isString(association)) {
    throw new Error('Expected an association parameter to be passed to ' +
      '`.populateCount(association, [keyName], [criteria])`');
  }

  if (_.isObject(keyName)) {
    criteria = keyName;
    keyName = null;
  }

  if (!_.isString(keyName)) {
    keyName = association + 'Count';
  }
  ...
  try {
    ...
    throw new Error('Could not parse sub-criteria passed to ' +
      util.format('`.populateCount("%s", "%s")`', association, keyName) + '\nSub-criteria:\n' +
      util.inspect(criteria, false, null) + '\nDetails:\n' + util.inspect(e, false, null));
  }

   // Get the current collection's primary key attribute
  Object.keys(self._context._attributes).forEach(function(key) {
    if(hasOwnProperty(self._context._attributes[key], 'primaryKey') && self._context._attributes[key].primaryKey) {
      pk = self._context._attributes[key].columnName || key;
    }
  });
  ...
  // Set the attr value to the generated schema attribute
  attr = self._context.waterline.schema[self._context.identity].attributes[association];

  if (!attr) {
    throw new Error('In ' + util.format('`.populate("%s", "%s")`', association, keyName) +
      ', attempting to populate an attribute that doesn\'t exist');
  }
  ...
  // If it's a belongs_to the adapter needs to know that it should replace the foreign key
  // with the associated value.
  parentKey = self._context.waterline.collections[self._context.identity].attributes[association];

  // Build the initial join object that will link this collection to either another collection
  // or to a junction table.
  count = {
    parent: self._context.identity,
    parentKey: attr.columnName || pk,
    child: attr.references,
    childKey: attr.on,
    alias: association,
...
Deferred.prototype.exec = function(cb) {
  ...
  if (this._criteria && _.isArray(this._criteria.select)) {
    var pk;
    _.keys(this._context._attributes).forEach(function(key) {
      if(hasOwnProperty(this._context._attributes[key], 'primaryKey') && this._context._attributes[key].primaryKey) {
        pk = this._context._attributes[key].columnName || key;
      }
    }, this);

    pk && (this._criteria.select.indexOf(pk) === -1) && this._criteria.select.unshift(pk);
  }
  ...
  args[0] = function (err, result) {
    ...
    if (!err && _.isObject(result)) {
      async.eachSeries(self._counts, function (item, next) {
        result[0][item.keyName] = 0;
        //console.log(util.inspect(item));

        next();
      }, function (err) {
        err && !resultArgs[0] && (resultArgs[0] = err); // push count error
        cb.apply(scope, resultArgs);
      });
    }
  };

  // add the criteria and values
  this._values && args.unshift(this._values);
