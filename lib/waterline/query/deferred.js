...
Deferred.prototype.populate = function(keyName, criteria) {
  ...
  try {
    ...
    Object.keys(this._context.waterline.schema[attr.references].attributes).forEach(function(key) {
      // if user select, then exclude ones not in selection
      if (userSelect && userSelect.indexOf(key) === -1) {
        return;
      }

      var obj = self._context.waterline.schema[attr.references].attributes[key];
      select.push(!hasOwnProperty(obj, 'columnName') ? key : obj.columnName);
    });
    ...
    if(this._context.waterline.schema[attr.references].junctionTable && hasOwnProperty(attr, 'on')) {
      ...
      if(ref) {
        ...
        var selects = _.compact(_.map(_.keys(this._context.waterline.schema[reference[ref].references].attributes), function(attr) {
          if (userSelect && userSelect.indexOf(attr) === -1) { return; } // if user select, then exclude ones not in selection
