...
module.exports = function(values, cb, _transactionId) {

  var self = this;

  // Handle Deferred where it passes criteria first
  if(typeof _transactionId !== 'string' && arguments.length === 3) {
  ...
  if(typeof cb !== 'function') {
    return new Deferred(this, this.create, {}, values, undefined, _transactionId);
  }
  ...
  createBelongsTo.call(this, valuesObject, function(err) {
    ...
    beforeCallbacks.call(self, valuesObject, function(err) {
      if(err) return cb(err);
      createValues.call(self, valuesObject, cb, _transactionId);
    });
  });
};
...
function createValues(valuesObject, cb, _transactionId) {
