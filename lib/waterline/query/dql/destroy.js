...
module.exports = function(criteria, cb, _transactionId) {
  var self = this,
      pk;
  ...
  if(typeof cb !== 'function') {
    return new Deferred(this, this.destroy, ((!criteria.transactionId && _transactionId) &&
      (criteria.transactionId = _transactionId), criteria));
  }
  ...
  callbacks.beforeDestroy(self, criteria, function(err) {
    ...
    _transactionId && criteria && (criteria.transactionId = _transactionId);

    function destroyRecords (err, result) {
      ...
      function destroyJoinTableRecords(item, next) {
        ...
        if(mappedValues.length > 0) {
          ...
          if (collection.destroyMode === 'soft') {
            var values = { deleted: true };
            _transactionId && (values.transactionId = _transactionId);
            collection.update(criteria, values).exec(next);
          }
          else {
            collection.destroy(criteria, undefined, _transactionId).exec(next);
          }
        } else {
          return next();
        }
    ...
    // In case of soft delete, update the action to update instead of destroy
    if (self.destroyMode === 'soft') {
      var values = { deleted: true };
      _transactionId && (values.transactionId = _transactionId);
      self.update(criteria, values, destroyRecords, _transactionId);
    }
    // Pass to adapter
    else {
      self.adapter.destroy(criteria, destroyRecords);
    }
  });
};
