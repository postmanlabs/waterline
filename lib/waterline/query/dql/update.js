...
function updateRecords(valuesObject, cb) {
  ...
  self.adapter.update(valuesObject.criteria, valuesObject.values, function(err, values, originals) {
    ...
    // prepare the original data as returned from database
    if(!Array.isArray(originals)) originals = originals && [originals] || [];
    var changesets = transformedValues.map(function (value, index) {
      if (!originals[index]) { return {}; } // better safe than crash!
      return self._transformer.unserialize(originals[index]);
    });
    ...
    updatedNestedAssociations.call(self, valuesObject, transformedValues, function(err) {
      ...
      async.each(transformedValues, function(record, callback) {
        ...
        var deltas = changesets.map(function(value, i) {
          var model = models[i],
            original = (model && value) ? new self._model(value) : {};

          // retain only the keys that changed
          _.each(_.keys(original), function (key) {
            _.isEqual(original[key], model[key]) && (delete original[key]);
          });

          return original;
        });

        cb(null, models, deltas);
      });
    });
