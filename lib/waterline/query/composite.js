...
module.exports = {
  ...
  findOrCreate: function(criteria, values, cb, _transactionId) {
    var self = this;
    ...
    if(typeof cb !== 'function') {
      return new Deferred(this, this.findOrCreate, (_transactionId && (criteria.transactionId = _transactionId),
        criteria), values);
    }

    // un-inject from criteria
    ...
    if(Array.isArray(criteria) && Array.isArray(values)) {
      return this.findOrCreateEach(criteria, values, cb, _transactionId);
    }

    if(typeof cb !== 'function') return usageError('Invalid callback specified!', usage, cb);

    // Try a find first.
    this.find((_transactionId && (criteria.transactionId = _transactionId), criteria)).exec(function(err, results) {
      ...
      self.create((_transactionId && values && (values.transactionId = _transactionId), values)).exec(function(err, result) {
        if(err) return cb(err);
