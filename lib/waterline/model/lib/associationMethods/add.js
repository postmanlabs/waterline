...
var Add = module.exports = function(collection, proto, records, cb) {
  ...
  this.createCollectionAssociations(records, cb, proto.transactionId);
};
...
Add.prototype.createCollectionAssociations = function(records, cb, _transactionId) {
  var self = this;

  async.eachSeries(Object.keys(records), function(associationKey, next) {
    self.createAssociations(associationKey, records[associationKey], next, _transactionId);
  },
...
Add.prototype.createAssociations = function(key, records, cb, _transactionId) {
  var self = this;
  ...
  async.eachLimit(records, 10, function(association, next) {
    ...
    if(association !== null && typeof association === 'object' && Object.keys(association).length > 0) {
      ...
      if(hasOwnProperty(association, associatedCollection.primaryKey)) {
        ...
        return self.updateRecord(associatedCollection, schema, association[pk], next, _transactionId);
      }

      return self.createNewRecord(associatedCollection, schema, association, next, _transactionId);
    }
    ...
    self.updateRecord(associatedCollection, schema, association, next, _transactionId);
...
Add.prototype.createNewRecord = function(collection, attribute, values, cb, _transactionId) {
  var self = this;
  ...
  _transactionId && (values.transactionId = _transactionId);
  ...
  collection.create(values, function(err, record) {
    ...
    self.createManyToMany(joinCollection, attribute, record[primaryKey], cb, _transactionId);
  });
};
...
Add.prototype.updateRecord = function(collection, attribute, values, cb, _transactionId) {
  var self = this;
  ...
  if(junctionTable) {
    ...
    return this.createManyToMany(joinCollection, attribute, values, cb, _transactionId);
  }
  ...
  _transactionId && (_values.transactionId = _transactionId);
...
Add.prototype.createManyToMany = function(collection, attribute, pk, cb, _transactionId) {
  var self = this;
