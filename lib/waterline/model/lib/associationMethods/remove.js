...
var Remove = module.exports = function(collection, proto, records, cb) {
  ...
  this.removeCollectionAssociations(records, cb, proto.transactionId);
};
...
Remove.prototype.removeCollectionAssociations = function(records, cb, _transactionId) {
  var self = this;

  async.eachSeries(Object.keys(records), function(associationKey, next) {
    self.removeAssociations(associationKey, records[associationKey], next, _transactionId);
  },
...
Remove.prototype.removeAssociations = function(key, records, cb, _transactionId) {
  var self = this;
  ...
  async.eachLimit(records, 10, function(association, next) {
    self.removeRecord(associatedCollection, schema, association, next, _transactionId);
  }, cb);
...
Remove.prototype.removeRecord = function(collection, attribute, values, cb, _transactionId) {
  var self = this;
  ...
  if(junctionTable) {
    ...
    return this.removeManyToMany(joinCollection, attribute, values, cb, _transactionId);
  }
...
Remove.prototype.removeManyToMany = function(collection, attribute, pk, cb, _transactionId) {
  var self = this;
