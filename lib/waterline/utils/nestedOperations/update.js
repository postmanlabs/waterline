...
function buildOperations(associations, values) {
  var self = this;
  var operations = values.transactionId && (function () {
      var operationsInterface = function () {};
      operationsInterface.prototype.transactionId = values.transactionId;
      return new operationsInterface();
    }()) || {};
...
function unlinkRunner(operations, cb) {
  ...
  function unlinkParentAssociations(parent, next) {
    ...
    operations.transactionId && (opts.transactionId = operations.transactionId);
...
function removeOperationRunner(operations, cb) {
  ...
  function runner(operation, next) {
    ...
    if(!operation.nullify) {
      self.waterline.collections[operation.model].destroy(operation.criteria, undefined, operations.transactionId)
        .exec(next);
      return;
    }
